...
class Module:
    ...
    def output(self, end_line_no):
        ...
        with open(self.name_v, 'w', encoding='utf-8') as f:
            if self.name == 'VC707PCIeShell':
                print('`define RANDOMIZE_GARBAGE_ASSIGN', file=f, flush=True)
                print('`define RANDOMIZE_INVALID_ASSIGN', file=f, flush=True)
                print('`define RANDOMIZE_REG_INIT',       file=f, flush=True)
                print('`define RANDOMIZE_MEM_INIT',       file=f, flush=True)
                print('`define RANDOM 0',                 file=f, flush=True)
